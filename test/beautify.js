...
describe('resource-policy-compiler -> beautify policy', function () {
  ...
  it('beautify single policy', function (done) {
    ...
    assert.equal(result.replace(/\s/g, ""), readSync(path.join(base, 'beautify')).replace(/\s/g, ""))
    done()
  })
  ...
  it('beautify terminate policy', function (done) {
    ...
    var result = compiler.beautify(policy)
    assert.equal(result.replace(/\s/g, ""), readSync(path.join(base, 'terminate-beautify')).replace(/\s/g, ""))
    done()
  })
...
`
    ...
    var result = compiler.beautify(policy)
    assert.equal(result.replace(/\s/g, ""), readSync(path.join(base, 'multi-beautify')).replace(/\s/g, ""))
    done()
  })
});
